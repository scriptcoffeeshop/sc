...
import { applyCorsHeaders, jsonResponse, corsHeaders } from './utils/cors.ts'
...
serve(async (req: Request) => {
    ...
    try {
        switch (action) {
            ...
            case 'linePayCancel': result = await linePayCancel(data, req); break
            ...
            case 'verifyAdmin': {
                ...
                result = a ? { success: true, isAdmin: a.isAdmin, role: a.role, message: 'OK' } : { success: false, isAdmin: false, message: '請先登入' }
                break
            }
        ...
        if (msg.includes('登入') || msg.includes('權限') || msg.includes('Token') || msg.includes('無效')) {
            return applyCorsHeaders(jsonResponse({ success: false, error: msg }, 401), req)
        }
