...
export async function updateUserRole(data: Record<string, unknown>, req: Request) {
    await requireSuperAdmin(req)
    ...
    // 目前簡化處理
    const { error } = await supabase.from('coffee_users').update({ role: String(role) }).eq('line_user_id', String(userId))
    if (error) return { success: false, error: error.message }
...
export async function getBlacklist(req: Request) {
    ...
    const { data, error } = await supabase.from('coffee_users').select('*').not('blocked_at', 'is', null)
    if (error) return { success: false, error: error.message }
    return { success: true, blacklist: data || [] }
}

export async function addToBlacklist(data: Record<string, unknown>, req: Request) {
    await requireAdmin(req)
...
export async function removeFromBlacklist(data: Record<string, unknown>, req: Request) {
    await requireAdmin(req)
    const { userId } = data
    if (!userId) return { success: false, error: '缺少 userId' }
    const { error } = await supabase.from('coffee_users').update({
