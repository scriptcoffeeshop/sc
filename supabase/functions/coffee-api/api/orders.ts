...
import { extractAuth, requireAuth, requireAdmin } from '../utils/auth.ts'
import {
    FRONTEND_URL,
    SMTP_USER,
...
export async function submitOrder(data: Record<string, unknown>, req: Request) {
    ...
    const { data: settingsData } = await supabase.from('coffee_settings')
        .select('key, value')
        .in('key', ['delivery_options_config', 'payment_routing_config', 'linepay_enabled', 'transfer_enabled'])
    ...
    settingsData?.forEach((r: any) => {
        if (r.key === 'delivery_options_config') {
            try { deliveryConfig = JSON.parse(r.value) } catch (e) { }
        } else if (r.key === 'payment_routing_config') {
            try { routingConfig = JSON.parse(r.value) } catch (e) { }
    ...
    if (!deliveryConfig.length) {
        ...
        deliveryConfig = [
            { id: 'in_store', enabled: true, payment: routingConfig['in_store'] || { cod: true, linepay: false, transfer: false } },
            { id: 'delivery', enabled: true, payment: routingConfig['delivery'] || { cod: true, linepay: false, transfer: false } },
            { id: 'home_delivery', enabled: true, payment: routingConfig['home_delivery'] || { cod: true, linepay: false, transfer: false } },
            { id: 'seven_eleven', enabled: true, payment: routingConfig['seven_eleven'] || { cod: true, linepay: false, transfer: false } },
            { id: 'family_mart', enabled: true, payment: routingConfig['family_mart'] || { cod: true, linepay: false, transfer: false } }
        ]
    }
    ...
    if (data.email) {
        const methodMap: Record<string, string> = { delivery: '配送到府', home_delivery: '全台宅配', seven_eleven: '7-11 取貨/取貨付款', family_mart: '全家取貨/取貨付款', in_store: '來店自取' }
        const paymentMap: Record<string, string> = { cod: '貨到付款', linepay: 'LINE Pay', transfer: '銀行轉帳' }
        const isDelivery = deliveryMethod === 'delivery' || deliveryMethod === 'home_delivery'
        const deliveryText = isDelivery ? `${data.city}${data.district} ${data.address}` : `${data.storeName} (${data.storeAddress})`
...
<div style="font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 10px rgba(0,0,0,0.1); border: 1px solid #e5ddd5;">
  ...
  <div style="background-color: #f5f5f5; color: #888888; text-align: center; padding: 15px; font-size: 12px; border-top: 1px solid #eeeeee;">
    ...
    if (paymentMethod === 'linepay') {
        try {
            const confirmUrl = `${FRONTEND_URL}/main.html?lpAction=confirm&orderId=${orderId}`
            const cancelUrl = `${FRONTEND_URL}/main.html?lpAction=cancel&orderId=${orderId}`
...
export async function updateOrderStatus(data: Record<string, unknown>, req: Request) {
    await requireAdmin(req)
    ...
    if (data.status === 'shipped' && orderData?.email) {
        ...
        const isDelivery = orderData.delivery_method === 'delivery' || orderData.delivery_method === 'home_delivery'
        ...
        if (data.trackingNumber) {
            ...
            if (orderData.delivery_method === 'seven_eleven') {
                trackingLink = `<a href="https://eservice.7-11.com.tw/e-tracking/search.aspx" target="_blank" style="display:inline-block; margin-top:8px; padding:6px 12px; background-color:#1e40af; color:#ffffff; text-decoration:none; border-radius:4px; font-size:13px;">🔗 7-11 貨態查詢</a>`
            } else if (orderData.delivery_method === 'family_mart') {
                trackingLink = `<a href="https://fmec.famiport.com.tw/FP_Entrance/QueryBox" target="_blank" style="display:inline-block; margin-top:8px; padding:6px 12px; background-color:#059669; color:#ffffff; text-decoration:none; border-radius:4px; font-size:13px;">🔗 全家貨態查詢</a>`
            } else if (isDelivery) {
                trackingLink = `<a href="https://postserv.post.gov.tw/pstmail/main_mail.html?targetTxn=EB500100" target="_blank" style="display:inline-block; margin-top:8px; padding:6px 12px; background-color:#047857; color:#ffffff; text-decoration:none; border-radius:4px; font-size:13px;">🔗 中華郵政貨態查詢</a>`
            }
            trackingSection = `<p style="margin: 10px 0 0 0; padding-top: 10px; border-top: 1px dashed #dcd3cb;"><strong>物流單號：</strong> <span style="font-family:monospace; font-size:15px; font-weight:bold;">${sanitize(String(data.trackingNumber))}</span><br>${trackingLink}</p>`
        }
