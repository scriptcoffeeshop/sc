...
test('check pm-category DOM', async ({ page }) => {
    ...
    await page.evaluate(() => {
        localStorage.setItem('coffee_admin', JSON.stringify({ userId: 'test', displayName: 'Admin' }));
    ...
    await page.route('**/coffee-api*', async route => {
        ...
        if (action === 'getCategories') {
            ...
            await route.fulfill({ json: { success: true, categories: [{ id: 1, name: "單品豆" }, { id: 2, name: "配方豆" }] } });
        } else if (action === 'getProductsAdmin') {
            await route.fulfill({ json: { success: true, products: [{ id: 1, name: 'VOID 配方豆', category: '配方豆', specs: '[]', order_index: 0, enabled: true }] } });
        } else {
