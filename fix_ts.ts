...
function rep(subpath: string, searchItems: [string, string][]) {
  const p = `./supabase/functions/coffee-api/${subpath}`;
  try {
    let code = readTextFileSync(p);
    for (const [from, to] of searchItems) {
      code = code.replaceAll(from, to);
    }
    writeTextFileSync(p, code);
  } catch (e) {
...
rep("api/payments.ts", [
  ...
  [
    "const refundRes = await linePayRequest(",
...
rep("api/orders.ts", [
  ...
  [
    "const lpRes = await linePayRequest(",
