...
- 訂單狀態變更可寫入 `coffee_orders.status_note` 作為最新狀態通知備註；後台訂單卡片/批次工具列、Email 通知、LINE Flex 通知與前台我的訂單會顯示同一份內容。狀態更新成功後由後端自動寄出消費者 Email 與 LINE Flex 通知，不會開啟前端 LINE Flex 一鍵發送視窗；手動 LINE 通知也會直接送出並以 toast 回報。
...
- 訂單狀態變更新增「給消費者的狀態備註」欄位，逐筆/批次更新會保存到 `status_note`，後端會以更新後的訂單資料自動寄出消費者 Email 與 LINE Flex 狀態通知，前台我的訂單也會顯示最新狀態備註；逐筆/批次備註欄位固定顯示，未選新狀態前停用提示，狀態更新與手動 LINE 通知都不再跳出 LINE Flex 預覽視窗。
