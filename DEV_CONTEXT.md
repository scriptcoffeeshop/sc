...
- Supabase Edge Function 的 `utils/json.ts` 已改成共享 `JsonRecord` 介面；request/auth/users 工具、categories/promotions/bank accounts/form fields/users/profile/transfer、quote 報價核心、payment shared/expiry、order admin/products 與 LINE Pay/JKO provider 已開始改用命名 JSON 邊界，後續再收斂產品設定、送單查單與 LINE template 等模組。
