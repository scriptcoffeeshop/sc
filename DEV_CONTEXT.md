...
- `ci-local` 已串入前端 `typecheck` baseline（`frontend/tsconfig.typecheck.json`），目前先覆蓋共享型別、共用 lib helper、UI base components、dashboard LINE Flex 訊息組裝、訂單狀態控制器、分類、活動與匯款帳號 composable；完整前端型別檢查保留在 `typecheck:full`，仍有 storefront / dashboard legacy TS 遷移債，後續應逐步擴大 baseline include 範圍。
...
- 前端型別守門已開始落地：新增 `vue-tsc` 與 `frontend/tsconfig.typecheck.json`，`npm run typecheck` 先檢查共享型別、共用 lib helper、UI base components、dashboard LINE Flex 訊息組裝、訂單狀態控制器、分類、活動與匯款帳號 composable，並已串入 `ci-local`；另修正 dashboard 初始載入 Promise 型別，讓 `npm run typecheck:full` 的 dashboard bootstrap / status controller / categories / promotions / bank accounts 錯誤消失；`npm run typecheck:full` 保留作為完整前端 TS 清債入口。
