...
- 後台是優先遷移區：
  ...
  - `frontend/tsconfig.json` 已啟用 `noImplicitAny`、`noImplicitThis`、`strictBindCallApply`、`strictFunctionTypes`、`alwaysStrict`、`useUnknownInCatchVariables` 與 `noFallthroughCasesInSwitch`；production 與 unit test helper 都不得再靠隱性 any、過寬 callback 邊界或未收斂的 catch error 通過型別檢查。完整 strict 仍採漸進式推進。
...
- `frontend/tsconfig.json` 已啟用 `noImplicitAny`、`noImplicitThis`、`strictBindCallApply`、`strictFunctionTypes`、`alwaysStrict`、`useUnknownInCatchVariables` 與 `noFallthroughCasesInSwitch`；前端新增參數、測試 fixture、動態 import 模組、全域 bridge 與服務注入 callback 都要補明確型別。下一個可收斂方向是分階段打開 `strictNullChecks`。
