...
- 新增「取消原因」流程與通知能力：
  - 訂單狀態改為 `cancelled` 時，後台會要求填寫取消原因。
  - `coffee_orders` 新增 `cancel_reason` 欄位（migration：`202604131000_add_order_cancel_reason.sql`）。
  - 訂單列表會顯示取消原因，並可帶入手動 LINE Flex 通知內容。
  - `sendOrderEmail` 新增 `cancelled` 模式，支援寄送「訂單取消通知」信件並顯示取消原因。
- 補齊手動通知操作：
  - Vue 後台訂單卡新增「發送信件」按鈕，轉帳/LINE Pay 收款後仍可手動發送 Flex 與 Email。
- 後台操作體驗優化：
  - 商品管理與促銷活動管理的「啟用 / 未啟用」改為可直接點擊文字一鍵切換。
  - 登入卡標題改為「後台登入」，並改用灰黑色文字。
