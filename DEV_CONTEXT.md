...
- 後台是優先遷移區：
  ...
  - `settings` / `formfields` / `categories` / `products` / `promotions` / `users` / `blacklist` 的按鈕互動已改成元件事件直連；`products` / `promotions` modal 儲存也已改成元件內 submit，不再依賴 `createSettingsActionHandlers()`、`createUsersActionHandlers()`、`createProductsActionHandlers()` 或 `product-form` / `promotion-form` 的 imperative submit listener
  - 剩餘的主要 legacy orchestration 集中在 dashboard bootstrap / window globals，以及 `orders` 為主的 document-level delegation
