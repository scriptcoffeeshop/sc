...
- 後台是優先遷移區：
  ...
  - `frontend/tsconfig.json` 已啟用完整 `strict: true`，並額外啟用 `noFallthroughCasesInSwitch`；production 與 unit test helper 都不得再靠隱性 any、過寬 callback 邊界、未收斂 nullable 或未收斂的 catch error 通過型別檢查。
...
- `frontend/tsconfig.json` 已啟用完整 `strict: true`，並額外啟用 `noFallthroughCasesInSwitch`；前端新增參數、測試 fixture、動態 import 模組、全域 bridge、服務注入 callback 與 nullable 資料邊界都要補明確型別。下一個可收斂方向是評估 `noUncheckedIndexedAccess`、`exactOptionalPropertyTypes`、`noPropertyAccessFromIndexSignature` 的成本。
