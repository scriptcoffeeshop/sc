...
- `ci-local` 已串入前端 `typecheck` baseline（`frontend/tsconfig.typecheck.json`），目前先覆蓋共享型別、共用 lib helper、UI base components 與 dashboard LINE Flex 訊息組裝；完整前端型別檢查保留在 `typecheck:full`，仍有 storefront / dashboard legacy TS 遷移債，後續應逐步擴大 baseline include 範圍。
...
- 前端型別守門已開始落地：新增 `vue-tsc` 與 `frontend/tsconfig.typecheck.json`，`npm run typecheck` 先檢查共享型別、共用 lib helper、UI base components 與 dashboard LINE Flex 訊息組裝，並已串入 `ci-local`；`npm run typecheck:full` 保留作為完整前端 TS 清債入口。
