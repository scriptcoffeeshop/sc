...
- Supabase Edge Function 的 `utils/json.ts` 已改成共享 `JsonRecord` 介面；categories/promotions/bank accounts/form fields/users/profile/transfer、quote 報價核心、payment shared/expiry 已開始改用命名 JSON 邊界，後續再收斂 order admin、金流 provider 與產品設定等模組。
