...
- 前台 `MainPage.vue` 已存在，但 legacy `main.html` / `js/main-app.js` 仍是相容層的一部分；其中登入／我的訂單／會員資料／登出入口，以及公告關閉、付款切換、我的訂單關閉與轉帳帳號互動，已先改成 Vue 元件事件或區域 DOM listener，不再必須依賴 body-level click delegation。
