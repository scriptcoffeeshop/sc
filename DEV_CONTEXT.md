...
- 後台是優先遷移區：
  ...
  - `frontend/tsconfig.json` 已啟用完整 `strict: true`，並額外啟用 `exactOptionalPropertyTypes`、`noFallthroughCasesInSwitch`、`noPropertyAccessFromIndexSignature`、`noUncheckedIndexedAccess` 與 `noUnusedLocals`；production 與 unit test helper 都不得再靠隱性 any、過寬 callback 邊界、未收斂 nullable、未收斂的 optional/undefined 欄位、未確認的索引讀取、模糊的 index-signature dot access、未收斂的 catch error 或未使用的本地宣告通過型別檢查。
...
- `products` 模組也開始收斂：`useDashboardProducts.ts` 的規格 clone、商品 view model/grouping、product form reset/fill、save payload 組裝已拆到 `dashboardProductsShared.ts`
  ...
  - dashboard LINE Flex Message 預覽與歷史紀錄彈窗已改掛 Vue `DashboardFlexMessagePreview.vue` / `DashboardFlexHistoryList.vue`；`dashboardOrderFlexController.ts` 不再拼 HTML 或對 SweetAlert popup 手動綁事件。
  - dashboard 訂單 Email 發送確認與一般狀態變更確認已改掛 Vue `DashboardOrderEmailConfirm.vue` / `DashboardOrderStatusChangeConfirm.vue`；`dashboardOrderEmailController.ts` 與 `dashboardOrderStatusController.ts` 不再用 SweetAlert `html` 字串拼接訂單資料。
...
- 後端訂單狀態、配送方式、付款方式與付款狀態 label 已集中到 `utils/order-labels.ts`；Email 與 LINE Flex 保留各自長短文案差異，並以 `order_labels_test.ts` 覆蓋。
...
- storefront 頁面 body class 與購物車抽屜 scroll lock 已集中到 `storefrontBodySideEffects.ts`，`MainPage.vue` 不再直接讀寫 `document.body`。
- storefront 轉帳下單成功 SweetAlert HTML 已抽到 `buildTransferOrderSuccessDialogOptions()`，送單主流程不再拼接匯款帳號 HTML，並補上轉義測試。
- storefront 確認訂單 SweetAlert 已改掛載 Vue `StorefrontOrderConfirmSummary.vue`，不再用字串模板拼訂單摘要 HTML。
- storefront 付款狀態 / 付款跳轉 / 轉帳成立提示已改共用 Vue `StorefrontPaymentDialogSummary.vue` 掛載，付款彈窗不再拼接 HTML 字串。
...
- Supabase Edge Function 的 `utils/json.ts` 已改成共享 `JsonRecord` 介面；routing 入口/action map、request/auth/users 工具、categories/promotions/bank accounts/form fields/users/profile/transfer、settings schema/action、quote 報價核心、payment shared/expiry、order admin/products、order submit/query、超商地圖 callback、LINE template/messaging 與 LINE Pay/JKO provider 已改用命名 JSON 邊界；production backend 已清掉 `Record<string, unknown>` 型別殘留。
