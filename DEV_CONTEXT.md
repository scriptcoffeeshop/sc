...
- 後台是優先遷移區：
  ...
  - `frontend/tsconfig.json` 已啟用 `noImplicitAny`、`noImplicitThis`、`strictBindCallApply`、`strictFunctionTypes`、`strictNullChecks`、`alwaysStrict`、`useUnknownInCatchVariables` 與 `noFallthroughCasesInSwitch`；production 與 unit test helper 都不得再靠隱性 any、過寬 callback 邊界、未收斂 nullable 或未收斂的 catch error 通過型別檢查。完整 strict 仍採漸進式推進。
...
- `frontend/tsconfig.json` 已啟用 `noImplicitAny`、`noImplicitThis`、`strictBindCallApply`、`strictFunctionTypes`、`strictNullChecks`、`alwaysStrict`、`useUnknownInCatchVariables` 與 `noFallthroughCasesInSwitch`；前端新增參數、測試 fixture、動態 import 模組、全域 bridge、服務注入 callback 與 nullable 資料邊界都要補明確型別。下一個可收斂方向是評估剩餘 strict family 與 `noUncheckedIndexedAccess` 的成本。
