...
- 後台是優先遷移區：
  ...
  - 共用 UI primitive (`Button/Input/Textarea/Card/Select`)、dashboard sections / settings cards / order cards / modals 與部分 storefront shell 元件已轉為 `<script setup lang="ts">`；目前剩餘未轉 SFC 主要集中在 pages 與 storefront 顯示元件。
