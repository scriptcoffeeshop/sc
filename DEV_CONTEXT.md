...
- 後台是優先遷移區：
  ...
  - `frontend/tsconfig.json` 已啟用完整 `strict: true`，並額外啟用 `exactOptionalPropertyTypes`、`noFallthroughCasesInSwitch` 與 `noUnusedLocals`；production 與 unit test helper 都不得再靠隱性 any、過寬 callback 邊界、未收斂 nullable、未收斂的 optional/undefined 欄位、未收斂的 catch error 或未使用的本地宣告通過型別檢查。
...
- `frontend/tsconfig.json` 已啟用完整 `strict: true`，並額外啟用 `exactOptionalPropertyTypes`、`noFallthroughCasesInSwitch` 與 `noUnusedLocals`；前端新增參數、測試 fixture、動態 import 模組、全域 bridge、服務注入 callback、nullable / optional 資料邊界與暫存宣告都要補明確型別或清除。下一個可收斂方向是評估 `noUncheckedIndexedAccess`、`noPropertyAccessFromIndexSignature` 的成本。
