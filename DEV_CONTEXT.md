...
- Supabase Edge Function 的 `utils/json.ts` 已改成共享 `JsonRecord` 介面；categories/promotions/bank accounts/form fields/users/profile/transfer 與 quote 報價核心已開始改用命名 JSON 邊界，後續再收斂 payment/order admin 等高耦合模組。
