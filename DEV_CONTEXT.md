...
- 後端 Email template 與 LINE Flex 的配送文字判斷已集中到共用 helper；訂單成立、出貨、處理中與 LINE 訂單狀態通知共用宅配/超商地址格式。
...
- 後端 Email template 與 LINE Flex 的配送文字判斷已集中到共用 helper；訂單成立、出貨通知、處理中通知與 LINE 訂單狀態通知不再各自拼接宅配/超商地址文案。
