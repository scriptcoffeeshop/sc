...
- 訂單狀態變更可寫入 `coffee_orders.status_note` 作為最新狀態通知備註；後台訂單卡片/批次工具列、Email 通知、LINE Flex 通知與前台我的訂單會顯示同一份內容。狀態更新成功後只保存狀態與備註，不會自動推送消費者 Email 或 LINE Flex，也不會開啟前端 LINE Flex 一鍵發送視窗；手動 LINE 通知仍會直接送出並以 toast 回報。
...
- 訂單狀態變更新增「給消費者的狀態備註」欄位，逐筆/批次更新會保存到 `status_note`，既有的手動 Email 與 LINE Flex 通知會讀取更新後的訂單備註，前台我的訂單也會顯示最新狀態備註；逐筆/批次備註欄位固定顯示，未選新狀態前停用提示，狀態更新與手動 LINE 通知都不再跳出 LINE Flex 預覽視窗。
...
- 前台「我的訂單」物流追蹤連結改為已出貨、已配達或已完成後才顯示，避免待處理超商取貨訂單露出預設查詢網址。
- 前台「我的訂單」的 7-11 / 全家取貨門市名稱同步顯示店號，`getMyOrders` 會回傳 `storeId` 給顧客端。
- 會員 LINE 登入顯示名稱固定保留 LINE profile 名稱；下單/會員表單的收件人姓名或暱稱改存到會員自訂欄位預設，不再覆蓋 LINE 顯示名。
- 後台登入前先預載並快取 `dashboard_title`，避免登入後頁首先閃出預設「咖啡訂購後台」再切換成自訂後台名稱。
- 後台出貨物流商預設選項改為 `7-11`、`全家`、`中華郵政`、`其他`，仍保留舊「貨態查詢」名稱回填相容。
- 後台訂單列表的 `Flex 歷史` 與 `重整` 操作補上游標、hover、pressed 與 focus-visible 回饋狀態。
